  ...
  return async (args, options = {}) => {
    return trySafe(async () => {
      ...
      const { url: _url, input } = generateRequest(
        `${client.endpoint}/api/${client.apiVersion}${client.pathPrefix}${path}`,
        {
          method,
          ...opts,
...
function resolve<T>(response: any): SafeReturn<T, ArgoCDError> {
  // It's a Json error response
  if (typeof response === 'object' && 'errors' in response) {
    return { error: new ArgoCDError(response.errors) };
  }
